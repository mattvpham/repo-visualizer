  ...
  "node_modules/react/cjs/react.development.js"(exports2) {
    ...
    if (process.env.NODE_ENV !== "production") {
      (function() {
        ...
        function createElement(type2, config2, children2) {
          var propName;
          var props2 = {};
          var key = null;
          var ref = null;
...
var processDir = async (rootPath = "", excludedPaths = [], excludedGlobs = []) => {
  ...
  const getFileStats = async (path = "") => {
    const stats = import_fs.default.statSync(`${path}`);
  ...
  const addItemToTree = async (path = "", isFolder = true) => {
    try {
      ...
      if (isFolder) {
        const filesOrFolders = import_fs.default.readdirSync(`${path}`);
        const children2 = [];
        for (const fileOrFolder of filesOrFolders) {
          ...
          const info = import_fs.default.statSync(`${fullPath}`);
          const stats3 = await addItemToTree(fullPath, info.isDirectory());
          if (stats3)
...
var main = async () => {
  ...
  await (0, import_child_process.exec)("git", ["config", "user.name", username]);
  await (0, import_child_process.exec)("git", [
    "config",
    "user.email",
  ...
  if (branch) {
    await (0, import_child_process.exec)("git", ["fetch"]);
    try {
      await (0, import_child_process.exec)("git", ["switch", "-c", branch, "--track", `origin/${branch}`]);
    } catch {
      doesBranchExist = false;
      core.info(`Branch ${branch} does not yet exist, creating ${branch}.`);
      await (0, import_child_process.exec)("git", ["checkout", "-b", branch]);
    }
  }
  ...
  if (shouldPush) {
    ...
    await (0, import_child_process.exec)("git", ["commit", "-m", commitMessage]);
    if (doesBranchExist) {
      await (0, import_child_process.exec)("git", ["push"]);
    } else {
      await (0, import_child_process.exec)("git", ["push", "--set-upstream", "origin", branch]);
    }
    if (branch) {
      await (0, import_child_process.exec)("git", "checkout", "-");
    }
    core.endGroup();
  }
  console.log("All set!");
